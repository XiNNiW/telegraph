...
//-----------------------------------------------------------------------------
//75D740F0 09DE4730 BE563C31 F4650CCF
FUID TelegraphSynthController::uid (0x75D740F0, 0x09DE4730, 0xBE563C31, 0xF4650CCF);
#else
//-----------------------------------------------------------------------------
//3DEEA41A 48E74E7B BAC0E603 1030BF06
FUID TelegraphSynthController::uid (0x3DEEA41A, 0x48E74E7B, 0xBAC0E603, 0x1030BF06);
#endif

//-----------------------------------------------------------------------------
TelegraphSynthController::TelegraphSynthController ()
{
...
//-----------------------------------------------------------------------------
tresult PLUGIN_API TelegraphSynthController::initialize (FUnknown* context)
{
	tresult res = BaseController::initialize (context);
	if (res == kResultTrue)
	{
		auto* presetParam = new IndexedParameter (
		    USTRING ("Factory Presets"), USTRING ("%"), TelegraphSynthProcessor::kNumPrograms - 1, 0,
		...
		parameters.addParameter (USTRING("resonator feedback"), USTRING(""), 0, 0.99, ParameterInfo::kCanAutomate, pid++);
		parameters.addParameter (USTRING("Attack"), USTRING(""), 0, 0.01, ParameterInfo::kCanAutomate, pid++);
		parameters.addParameter (USTRING("Decay"), USTRING(""), 0, 0.5, ParameterInfo::kCanAutomate, pid++);
		parameters.addParameter (USTRING("Sustain"), USTRING(""), 0, 0.777, ParameterInfo::kCanAutomate, pid++);
		parameters.addParameter (USTRING("Release"), USTRING(""), 0, 0.25, ParameterInfo::kCanAutomate, pid++);
		parameters.addParameter (USTRING("vibrato depth"), USTRING(""), 0, 0.0, ParameterInfo::kCanAutomate, pid++);
		parameters.addParameter (USTRING("vibrato speed"), USTRING(""), 0, 0.3, ParameterInfo::kCanAutomate, pid++);
		parameters.addParameter (USTRING("exciter tune"), USTRING(""), 5, 0.5, ParameterInfo::kCanAutomate, pid++);
		parameters.addParameter (USTRING("resonator left tune"), USTRING(""), 6, 0.5, ParameterInfo::kCanAutomate, pid++);
		parameters.addParameter (USTRING("lowpass filter q"), USTRING(""), 0, 0.5, ParameterInfo::kCanAutomate, pid++);
		parameters.addParameter (USTRING("fm tune"), USTRING(""), 7, 0.5, ParameterInfo::kCanAutomate, pid++);
		parameters.addParameter (USTRING("x feedback"), USTRING(""), 0, 0.5, ParameterInfo::kCanAutomate, pid++);
		parameters.addParameter (USTRING("exciter gain"), USTRING(""), 0, 0.5, ParameterInfo::kCanAutomate, pid++);
		parameters.addParameter (USTRING("fm amount"), USTRING(""), 0, 0.5, ParameterInfo::kCanAutomate, pid++);
		parameters.addParameter (USTRING("exciter wave shape"), USTRING(""), 2, 0, ParameterInfo::kCanAutomate, pid++);
		parameters.addParameter (USTRING("lowpass filter cutoff"), USTRING(""), 0, 0.5, ParameterInfo::kCanAutomate, pid++);
...
//-----------------------------------------------------------------------------
tresult PLUGIN_API TelegraphSynthController::terminate ()
{
	return BaseController::terminate ();
}

//-----------------------------------------------------------------------------
tresult PLUGIN_API TelegraphSynthController::setParamNormalized (ParamID tag, ParamValue value)
{
...
//-----------------------------------------------------------------------------
tresult PLUGIN_API TelegraphSynthController::getParamStringByValue (ParamID tag, ParamValue valueNormalized, String128 string)
{
...
//-----------------------------------------------------------------------------
tresult PLUGIN_API TelegraphSynthController::getParamValueByString (ParamID tag, TChar* string, ParamValue& valueNormalized)
{
